...
class HexunSpider(scrapy.spiders):
    name = 'sina'
    start_urls = ['https://weibo.com/ttarticle/p/show?id=2309404381896548211358l']
    ...
    def parse(self, response):
        ...
        # 格式为转发 55
        forward_num = forward_num[6:].strip()
        comment_num = li_list[1].find(class_="pos").span.string.strip()
        # 格式为评论 6
        comment_num = comment_num[6:].strip()
